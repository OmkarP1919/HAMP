...
<%
    ...
    String currentRoomNumber = "Not Allocated";
    String currentRoomNo = null; // Will store the room_no (String)
    ...
    try {
        ...
        String driver = "com.mysql.jdbc.Driver"; // Corrected driver name for newer MySQL Connector/J
        ...
        if (studRoll == null || studRoll.trim().isEmpty()) {
            ...
            if (rs.next()) {
                ...
                currentRoomNo = rs.getString("room_no"); // This gets the room_no if allocated
                if (currentRoomNo != null) {
                    currentRoomNumber = currentRoomNo; // For display, the room_no itself is the room number
                }
            } else {
            ...
            if ("POST".equalsIgnoreCase(request.getMethod())) {
                ...
                if ("allocate".equals(action) && studentFullname != null) {
                    String selectedRoomNo = request.getParameter("selected_room_no"); // Changed to room_no (String)
                    if (selectedRoomNo != null && !selectedRoomNo.trim().isEmpty()) {
                        try {
                            // Start Transaction
                            conn.setAutoCommit(false);

                            // 1. If student already has a room, de-allocate it first
                            if (currentRoomNo != null && !currentRoomNo.equals(selectedRoomNo)) { // Only deallocate if changing room
                                ...
                                pstmt.setString(1, currentRoomNo);
                                pstmt.executeUpdate();
                                pstmt.close();
                                ...
                                pstmt.executeUpdate();
                                pstmt.close();
                            } else if (currentRoomNo != null && currentRoomNo.equals(selectedRoomNo)) {
                                message = "Student is already allocated to room " + selectedRoomNo + ".";
                                messageType = "info"; // Not an error, just an info message
                                conn.rollback(); // Rollback any implicit changes if transaction was started
                                return; // Exit and display info message
                            }

                            // 2. Allocate the new room (or update if same room chosen after deallocation)
                            ...
                            if (rowsAffected > 0) {
                                // 3. Increment occupied_slots for the new room
                                ...
                                messageType = "success";
                                currentRoomNo = selectedRoomNo; // Update for display
                                currentRoomNumber = selectedRoomNo; // Update for display
                            } else {
            ...
            String availableRoomsSql = "SELECT room_no, floor_no, total_slots, occupied_slots " +
                                       "FROM room " +
                                       "WHERE occupied_slots < total_slots " + // Room is available if occupied slots < total slots
                                       "ORDER BY room_no";
...
<head>
    ...
    <style>
        :root {
            ...
            --info-color: #3b82f6; --info-bg: #eff6ff; --info-border: #93c5fd; /* Added info styling */
        ...
        * { box-sizing: border-box; }
        ...
        .message-box.info { background-color: var(--info-bg); border: 1px solid var(--info-border); color: var(--info-color); } /* Added info styling */
...
<body>
    ...
    <main class="main-content">
        ...
        <% if (studRoll != null && studentFullname != null) { %>
            <div class="card">
                ...
                <div class="detail-item">
                    ...
                    <span class="detail-value"><%= currentRoomNumber %>
                        <% if (currentRoomNo != null) { %>
                           (Room Number: <%= currentRoomNo %>)
                        <% } %>
                    </span>
                </div>
                ...
                <form action="admin_allocate_room.jsp?stud_roll=<%= studRoll %>" method="POST">
                    ...
                    <div class="form-actions">
                        <a href="admin_applications.jsp" class="button secondary"><i class="fas fa-arrow-left"></i> Back to Applications View</a>
                        <button type="submit" class="button primary" <%= availableRooms.isEmpty() ? "disabled" : "" %>>
        ...
        <% } else if (messageType == null || "error".equals(messageType)) { %>
            <div class="card">
                <p>Unable to load student details. Please return to the applications list.</p>
                <div class="form-actions">
                    <a href="admin_applications.jsp" class="button secondary"><i class="fas fa-arrow-left"></i> Back to Student List</a>
                </div>
            </div>
        <% } %>
    </main>
